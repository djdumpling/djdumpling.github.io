...
4. Based on the KL penalty and the value from the reward model (RM), we modify our former PPO objective: 
    ...
    $$L_t(\theta) = \hat{\mathbb{E}}_{x \sim X, y \sim \theta_\theta( \cdot \vert x)}\left[L_t^{\text{clip}}(\theta) - c_1 L_t^{VF}(\theta) + c_2 S[\pi_\theta](s_t) - \lambda_{KL}D_{KL}(\pi_{\theta}(y \vert x) \parallel \pi_{\text{base}}(y \vert x))\right] $$
5. 
