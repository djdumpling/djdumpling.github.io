...
4. Three values are calculated, including the logits and values derived from the experience. 
    ...
    Later, we'll denote this by $S[\pi_\theta](s_t)$, the entropy of the policy $\pi_\theta$ at state $s_t$. 
...
5. The three values calculated during step 4 are composed via a linear combination into the final objective: 
