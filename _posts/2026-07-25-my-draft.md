...
Seven mainline policy-gradient configurations produce no measured transferable improvement. Their bid logs still contain enough conditional structure to train a stronger model. Two runs track the frozen SFT policy directly and remain flat; the others include large gains against a scripted heuristic without establishing broader transfer. The rest of the article separates trajectory outcomes from conditional bid data, then accounts for the future value of cash.
...
For texture, let’s walk through examples of typical reasoning and how each relates to trading. The 6 decisions below all come from a single game, across 6 different rounds: 1, 9, 10, 11, 12, and 14. Some of the more routine rounds are skipped, however.
...
So, to you, the two-gem lot therefore has a private, pre-mission EV of about 21.91 points.

![Decision b, round 9: a blue-and-green lot completes two missions at once.](../public/public/megagem/decision_2.png)

**Missions create discontinuous value.** Before the auction, your collection is Red–Red–Blue. Winning Blue–Green changes it to Red–Red–Blue–Blue–Green: which completes two of the missions for 25 points total. Factoring in the blue and green gem from the lot and their value display, we know that winning this lot is worth at least $25+2\cdot4+2 \cdot4=41$. Then, considering that the maximum any other player has is 12, and that you are first in the tie-break order, you could win this auction for 12 and win at least $41-12=29$ in positive EV. We are able to benefit from the lack of liquidity is Alice and Bob to take this auction for much cheaper.

![Decision c, round 10: bidding from last in the tie-break order costs one extra coin.](../public/public/megagem/decision_3.png)

**Tie-break position is worth one bid increment.** Because you are last in the tie-breka order, so matching Alice’s bid is a losing action: to clear a predicted bid of $b$, you must bid $b+1$. The cash balances make the edge concrete: Alice can bid at most 12 and Bob at most 6, so 13 is the minimum bid to guaruntee the lot. However, this bid increase must still be compared with the gem’s value and the option value of keeping that coin for the remaining auctions, and potentially make the lot no longer worth bidding on.
...
**A reveal moves the market, not the terminal supply.** Both cards enter the Value Display at game end, so revealing Red or Purple does not change final gem values, but **it changes what the table is willing to pay before then**. Revealing Purple moves its public count from 2 to 3 and its current quote from 8 to 12. Bob already owns Yellow gems, so another Purple would also complete the 5-point Pendant mission: this reveal raises his visible reservation value for a future Purple from roughly $8+5=13$ to $12+5=17$. That can be useful if the goal is to induce Bob to spend his six coins, but it also makes a strategically important Purple harder to buy or deny, especially when you have only two coins and act last in ties.

Revealing Red has the opposite advantage. Moving Red’s quote from 4 to 8 cannot inflate the price of any remaining Red lot. It instead keeps Purple quoted at 8 while your hidden Purple guarantees that its terminal supply is higher than the table can see. You preserve a private-information edge over the live color, making a later Purple cheaper to snipe and Bob’s Pendant easier to block. Revealing Red does advertise more of your lead, but its direct price impact is confined to a market that has already closed.
...
**A loan is worth its liquidity, not its face value.** The 20 borrowed coins are matched by a 20-point repayment at scoring, so the principal itself creates no terminal value. Winning the loan for 10 changes your available cash to $C_{\mathrm{after}}=2+20-10=12$, but carrying those coins to the end would leave you 10 points worse off. The reason to you may want to borrow here is because that if you can win the auction for sufficiently cheap, you can effectively win the last few auctions for free. For example, if you win and have 14 coins, you can guaruntee winning the last two auctions by first paying 7 (beating Bob in the tiebreak) and paying 7 again (since you just won, you are last in the tiebreak and need to one-up Bob's liquidity of 6). Moreover, because you currently have the least liquidity, the loan should be worth the most to you. An argument against this is that loans are worth less because there are fewer auctions remaining to deploy the borrowed liquidity, and that if you bid too high, using the won surplus may not guaruntee positive EV.

Of the five cards still hidden, two are Purple, and three cards belong to rival hands while two are treasures. Let $X$ be the number of those two Purple cards in the three rival-hand slots, then $\Pr(X=x)=\frac{\binom{2}{x}\binom{3}{3-x}}{\binom{5}{3}}$. Therefore, $\bigl(\Pr[X=0],\Pr[X=1],\Pr[X=2]\bigr)=(0.1,0.6,0.3)$. Therefore, 
...
**f. Late-game bidding becomes auction poker.** Before the final lot, terminal-value accounting puts you at 77, Alice at 75, and Bob at 63. The Purple gem is worth 16 on Chart A; it would also complete Bob’s 5-point Pendant mission, but Bob has only 1 coin. The score branches are:
...
Your tie priority beats Bob, so a bid of 1 covers his branch. Alice, however, acts before you so 5 is the smallest bid that covers both stacks and produces a winning score of 88. Any bid from 6 through 12 buys the same gem while needlessly giving away score. The asset is fundamentally worth 16, but the correct price is 5 because the opponents’ remaining cash sets the clearing threshold.

## the option value of a coin

At face value, a coin contributes one point to final score. During the game, its real value is the best thing you can still do with it. You can buy the current lot, wait for a later auction where the coin goes further, sink it into an investment, or use it to deny an opponent a loan. That last trade is especially complicated with three players because starving one opponent can mainly help the third. You have the right but not the obligation to spend the coin, which makes liquidity option-like. As the live experiments later show, its marginal continuation value can be meaningfully greater than one.

Consider a per-auction expected-value bidder that treats coins only as their one-point terminal value. It bids up to each lot’s estimated value in isolation, ignoring that spending now removes the same coin from every later auction. This player tends to exhaust its budget by midgame and has little recourse unless a loan appears or its opponents also run out of cash. Mission rewards partially counteract that pressure because completing one can justify an aggressive bid. Our first selector includes exact mission value. It still fails because the mission bonus remains local while depleted liquidity affects the rest of the game.
...
Several well-known reinforcement-learning systems use self-play: AlphaGo and AlphaZero in Go and chess, AlphaStar in StarCraft II, OpenAI Five in Dota 2, Pluribus in poker, and KataGo in Go.

The broad recipe is to train an agent against copies or historical versions of itself and reward successful play. As the learner changes, its opponents change with it, which can create an automatic curriculum without a fixed external teacher. Self-play also works outside two-player zero-sum games. Its cleanest target and progress certificate appear inside that setting. Megagem removes those conveniences one by one.

There is a second complication: the trainable object is an LLM, not a policy head that emits one move. Consider AlphaGO again, which uses search with policy and value networks to select a single legal board action. However, in our setup, temperature sampling operates across an entire autoregressive response, so response entropy accumulates across token decisions before the parser turns text into a bid. That changes the learning problem: a game-level reward must assign credit through a long natural-language trajectory, many distinct responses can collapse to the same parsed bid, and probability mass can shift among reasoning or formatting choices without improving the underlying game action.
...
Under pure opposition, you must assume the opponent chooses whatever hurts you most. You therefore choose the strategy with the best worst case, the maximin. Your opponent does the converse, minimizing the maximum value you can obtain, the minimax. Von Neumann’s minimax theorem says these values are equal.
...
Mixed strategies matter because pure best responses can cycle. In rock-paper-scissors, every pure action is immediately exploitable, while the uniform mixture is not. The theorem lives over distributions so that the strategy space is convex and the opponent cannot exploit a deterministic pattern.

</details>

If neither player can improve by deviating alone, the joint policy is in Nash equilibrium. In a two-player zero-sum game, its equilibrium strategies are also minimax. This supplies a clean progress measure: exploitability, or how much a worst-case opponent could gain by deviating. A lower exploitability means the strategy is becoming harder for any opponent to exploit.

Ordinary policy-gradient self-play can still cycle in the latest policy. Practical systems therefore often average policies or train against historical snapshots, exploiters, and wider populations. Those mechanisms can stop the newest policy from forgetting old weaknesses. The underlying game still provides an opponent-independent target against which progress can be judged.

### two-player general-sum games

General-sum games remove the requirement that the players’ payoffs add to a constant in every outcome. Two subtypes show why this matters.

1. In mixed-motive games, interests are neither perfectly opposed nor perfectly aligned. In the prisoner’s dilemma, both players prefer mutual cooperation to mutual defection, but each is individually tempted to defect. Settlers of Catan has the same flavor through beneficial trades, temporary coalitions, and coordinated attempts to block a leader.
2. In fully cooperative games, every player receives the same payoff. Self-play can still learn arbitrary private conventions that fail when a new partner learned a different convention. Hanabi is the standard example: two policies can each be effective with copies of themselves yet coordinate poorly with one another.

General-sum rewards give each player a different value for the same position. Multiple equilibria may exist, and different learning populations can settle on different conventions or tradeoffs. Any improvement in a reward curve is indexed to the partners and opponents represented there.

### three-player general-sum

Megagem adds a third player. Score changes are not constrained to sum to zero, and an action can redistribute advantage among players in ways that barely affect the actor. Buying a loan to starve one opponent may mainly help the third. A losing player can decide which leader wins. Two players can pressure the leader without sharing a formal team reward.
...
Three additional properties make the learning problem harder.

First, deals are stochastic. A strong hand can raise terminal score without reflecting a better decision, just as pocket aces can make a poker hand profitable without proving that every action was skillful. Same-deal comparisons remove much of this variance. Credit assignment within a long trajectory remains unresolved.
...
This makes Megagem a partially observable Markov decision process (POMDP). The relevant value depends on the full belief state; the visible board alone is insufficient. This larger state space is harder to estimate from a lossy representation. A deterministic policy can also reveal exploitable regularities, so useful play may require action randomization.

Because bids are simultaneous, action selection also requires a distribution over clearing prices. For a candidate bid $b$, we need $P(\text{win}\mid b)$, the probability that the bid beats both opponents. Every bid trades the surplus retained after paying against the probability of winning. This is almost exactly the decision rule used later, where the selector combines an estimated lot value with a fitted distribution of opponent bids.

Third, terminal gem values are fixed by the initial deal but revealed endogenously through play. Every private gem eventually enters the public Value Display, so the final display does not change. The order of reveals changes what each player believes before the remaining auctions. A reveal can therefore move perceived value even when it does not move terminal value.

For the common-value component of a lot, winning also carries information. If both opponents decline to pay your price, you may have overvalued the asset, the auction version of the winner’s curse. Private mission value weakens that inference because the same lot can rationally be worth different amounts to different seats. The system eventually handles these pieces separately: a calibration term corrects systematic value bias, while a bid distribution represents uncertainty about the price the market will clear.
...
The environment emits complete game trajectories without defining the RL reward. A separate offline scorer reconstructs per-turn credit, and the update masks every opponent token so only the trainable seat receives gradients. Keeping the engine, training objective, and benchmark separate avoids making success circular.

## evaluation design

Three-player evaluation creates two different problems. Experimental design determines which triples play, on which deals, and in which seats. Estimation derives scalar Elo and confidence intervals. The benchmark uses three measurements that should not be conflated:
...
For production collection, we keep Flash and replace the screened Opus 4.5 with its newer 4.6 successor. We choose the successor based on model family; the screen itself tests only Opus 4.5. The final corpus uses 70% Flash and 30% Opus 4.6. Opus 4.6 averages about 421 visible words, estimated from 561 billed tokens, while the resulting corpus averages about 324 tokens under the student tokenizer. The table and those corpus statistics therefore use different units.
...
We select step 1,200 because it is fully legal and performs better across the frozen panel, including against Opus 4.6. The gains against easier opponents are already decisive. The three-copy self-play mean is only a sanity check: when every seat changes together, total score cannot show whether one checkpoint is stronger than another against a fixed opponent.
...
| long DAPO | +16.48 [14.26, 18.93] | not measured | <strong style="color:#cc0000">fail: max KL 0.646</strong> |
...
Megagem positions still have value. Under relative-margin self-play, all three copies benefit from the same favorable deal, which suppresses most common board advantage. The fixed-heuristic probe uses strategically different policies, allowing repeated outcomes to recover more of the position’s value.
...
Here, $\xi$ is the stochastic continuation generated by $\pi$ and $\omega$. We score every candidate on the same sampled worlds and random seeds, which reduces comparison noise without eliminating it. The estimate $Q$ still depends on $\omega$: changing the continuation opponent can reorder bids even when $(I,a)$ stays fixed.
...
The actor commits to a bid from information state $I$ before the opponents’ simultaneous bids are known. Once the auction resolves, those bids become public. During training, a critic can use this newly revealed information even though the actor never receives it at inference. This setup is called centralized training with decentralized execution. Holding the observed opponent bids $\mathbf b_{-i}^{\mathrm{obs}}$ fixed asks a narrow counterfactual: what happens if only our bid changes?
...
At each treasure-bid state reached by the actor, we sample $K=16$ bids and compress them to 6 diverse candidates. The selection retains the played bid while approximately preserving both the policy’s sampled mass and the numerical range of bids. We score every candidate on the same sampled worlds and random seeds, and hold the opponents’ observed bids fixed. This paired design reduces comparison noise. After the forced bid, a deterministic frozen SFT policy completes the game. The labels therefore apply to that frozen continuation policy; an updated PPO actor would induce a different distribution.
...
We reuse three confidence gates from calibration: gap $\ge0.05$, 95% sign confidence, and at most six pairs per state. A 4B critic starts from the SFT checkpoint. Its confidence-weighted Bradley–Terry loss ranks the better bid above the worse one, while a Huber regression term trains the numeric centered value at weight 0.25. Each one-epoch run uses 543 training pairs, 210 validation pairs, and a learning rate of $10^{-5}$.
...
The next step is to model the full bid distribution.
...
Zero regret can be unattainable when the budget cannot beat a profitable clearing price, or when $V>p$ but losing the tiebreak makes the cheapest winning bid $p+1$ instead of $p$. Even the best legal realized-price oracle can therefore have positive regret. The comparison covers the current sealed-bid auction conditional on the logged opponent bids. Future states and opponent adaptation stay fixed, so it does not estimate full-policy value.
...
| fitted mean + residual distribution | <strong style="color:#00aa00">2.819</strong> | <strong style="color:#00aa00">+1.304</strong> |
| best legal bid with realized prices | 2.118 | +2.006 |

We compute regret differences within each seed cluster and use those cluster means to form the $t$ statistic. The clairvoyant-value distributional selector beats the logged SFT bids by 1.304 coins per decision with $t=16.8$. Retaining residual uncertainty reduces regret by another 0.456 coins per decision relative to the point version using the *same fitted mean*. Both arms share that mean, so the contrast isolates the residual distribution.

The offline result covers one auction at a time. We next test full-game effects.
...
The previous ledger shows why the premium must be positive: the financing channels move by roughly $\frac{3.73+7.13}{6.81}\approx1.60$ points for each extra coin of treasure spending. That ratio is diagnostic, not an estimate of $\lambda_{\text{coin}}$, because the channels overlap and the intervention changes later states.

The dynamics simulator ranks candidates. Two archived calibration variants estimate the nominal baseline at −0.41 and +4.21 points; the sweep estimates +3.44. Those estimates disagree, and the absolute-calibration gate fails. We therefore screen settings for a plausible intervention footprint and reserve effect estimation for live evaluation.
...
The intervention converts a losing baseline into a winning one. The model-assisted adjustment is +12.62 [8.28, 16.96], close to the raw +12.16 [7.61, 16.71]. Pacing also preserves the extra asset value while removing the financing damage:
...
The selected bid of 13 still wins about 88% of the time. The final 12 percentage points of win probability cost too much, so the expert shades the bid to 13. Estimated paced utility changes from −2.41 to +9.79. Its stored training target reads:
...
The target names the local error and explains the price of the higher win probability. The log preserves neither the SFT policy’s free-form rationale nor a student response. This row therefore illustrates the supervision target; it provides no evidence that the student internalizes it.
...
Control wins 29.3% with margin −12.83. The distilled weights win 40.7% with margin −4.85. Raw paired improvement is +7.97 [2.70, 13.25], $t=2.96$; control-variate-adjusted improvement is +7.42 [2.56, 12.29], $t=2.99$. The preregistered gate requires a point estimate of at least +4 and a positive effect at $p<.05$; both conditions pass. Interval exclusion of +4 lies outside that criterion, and the lower bound of +2.56 cannot support the stronger claim. The 73% point-estimate ratio compares separate selector and distillation experiments.

The analytic branch’s first measured held-out improvement beyond SFT is weights-only. Its component ledger records treasure spend −0.63, investment return −0.57, and own final score +4.33 relative to control, with no recurrence of the liquidity cascade. Two implementation steps reduce the effect relative to the Flash-fitted analytic selector:
...
The optional selector produces +9.44 [4.55, 14.33] in a seat-rotated 150-pair test, with descriptive per-seat effects of +7.26, +10.94, and +10.12 over 50 pairs each. Against two Sonnet 4.5 opponents, a 50-pair screen is +25.12 [13.78, 36.46]. Against Gemini 3.1 Pro, it is +6.10 [−4.67, 16.87], directionally positive but underpowered. These tests evaluate the selector on top of the distilled policy; the weights-only artifact is unchanged.
...
Across separate experiments, adjusted point estimates leave a descriptive 2.99-point gap; the direct within-run selector increment is +3.95 [−0.83, 8.74]. Applying the two measured distillations’ 73–78% retention range to that gap suggests only 2.2–2.3 points from another round. The 2.2–2.3 figure is an extrapolation. The incremental live interval includes zero, and offline gates find the residual labels only weakly learnable. The expected effect does not justify another round.

## final benchmark

We next evaluate the Flash-centered gain in a broader field. The opening leaderboard ranks MegaGem-Distilled-4B first. Overlapping marginal intervals among the top three leave that ordering unresolved. In the 13-model Steiner-triple benchmark, direct comparisons resolve wins over 10 of 12 opponents; the focused test below adds a dedicated comparison with Gemini 3 Flash. Against SFT, Distilled finishes higher in 75% of shared games [58%, 88%], an internal check outside the development evaluations.
...
|---|---:|:---:|---:|---|
| Distilled vs Gemini 3 Flash | <strong style="color:#00aa00">0.56</strong> | <strong style="color:#00aa00">[0.52, 0.60]</strong> | +5.2 | Distilled win |
| Distilled vs Gemini 3.1 Pro | 0.53 | [0.48, 0.57] | +0.8 | unresolved |
| Gemini 3.1 Pro vs Gemini 3 Flash | <strong style="color:#00aa00">0.59</strong> | <strong style="color:#00aa00">[0.54, 0.65]</strong> | +4.4 | Pro win |

The focused test resolves Distilled above Gemini 3 Flash. Distilled versus Gemini 3.1 Pro remains unresolved.
...
Several configurations improve against a fixed heuristic while their frozen-SFT anchors stay flat. Stationary comparators are therefore essential: frozen SFT anchors, fixed-opponent gates, and non-co-adapting panels establish whether a moving reward transfers.

Three quantities matter in each bid. $\hat V$ estimates the asset, $\hat F$ estimates the market, and rollout $Q$ estimates a full continuation against specified policies. The selector combines value and price into a local action. Because bids are discrete, a point predictor with low average error still lands on the wrong side of the win/loss threshold in roughly a quarter of auctions, largely because many auctions clear within one coin.
...
Across these experiments, relative outcome rewards show no transfer across opponent populations. Conditional bid logs provide the useful learning signal: they expose price distributions, make liquidity cost explicit, and yield targets that can be distilled.
