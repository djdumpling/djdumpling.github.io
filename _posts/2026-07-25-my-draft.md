...
MegaGem is a multiplayer, imperfect-information auction game developed by Jane Street. In our three-player, general-sum experiments, self-play opponents and relative outcome rewards produce no measured transferable improvement: some policies learn opponent-specific exploits, while the runs with direct SFT anchors stay flat. The broader rollout infrastructure proved more useful as a data engine: policy-generated bid logs—from external-model and mixed-policy games as well as SFT play—supported a conditional market model. Starting from Qwen3-4B-Instruct, we train with SFT, build a paced distributional bidder, and distill its decisions through expert iteration. The final specialist wins direct paired comparisons against 10 of 12 benchmark opponents; in a separate top-three playoff, it beats Gemini 3 Flash and remains statistically unresolved against Gemini 3.1 Pro.

This blog would not be possible without the support of many people. I am especially grateful to [Johannes Hagemann](https://x.com/johannes_hage?lang=en), [Sebastian Müller](https://x.com/omouamoua), [Christian Reetz](https://x.com/creet_z), and [Peyton Walters](https://x.com/peywalt) for their generous support with compute and API usage.
...
- [distillation](#distillation)
  ...
  - [weights-only evaluation](#weights-only-evaluation)
    - [preregistered gate and model-assisted estimate](#preregistered-gate)
...
* **Figgie:** Jane Street’s open-outcry card game models commodities trading. Players see only their own cards, then buy and sell suits while trying to infer which commodity will pay out and what it is worth from limited supply information and the behavior of the market. Prices emerge as those private beliefs meet.
* **Splendor:** an engine-building game in which players collect gem tokens, spend them on development cards that make later purchases cheaper, and race to 15 prestige points. The shared cards and objectives create tension between improving your own position, preserving flexibility, and denying an opponent the resource or bonus they need.
...
Both cards enter the Value Display at game end, so revealing Red or Purple does not change final gem values, but **it changes what the table is willing to pay before then**. Revealing Purple moves its public count from 2 to 3 and its current quote from 8 to 12. Bob already owns Yellow gems, so another Purple would also complete the 5-point Pendant mission: this reveal raises his visible reservation value for a future Purple from roughly $8+5=13$ to $12+5=17$. That can be useful if the goal is to induce Bob to spend his six coins, but it also makes a strategically important Purple harder to buy or deny, especially when you have only two coins and act last in ties.
...
Several well-known RL systems use self-play: [AlphaGo](https://www.nature.com/articles/nature16961) in Go, [AlphaZero](https://arxiv.org/abs/1712.01815) in Go and chess, [AlphaStar](https://www.nature.com/articles/s41586-019-1724-z) in StarCraft II, [OpenAI Five](https://arxiv.org/abs/1912.06680) in Dota 2, [Pluribus](https://doi.org/10.1126/science.aay2400) in poker, and [KataGo](https://arxiv.org/abs/1902.10565) in Go.
...
<summary><strong>two-player zero-sum games</strong></summary>
...
General-sum rewards give each player a different value for the same position. Multiple equilibria may exist, and different learning populations can settle on different conventions or tradeoffs.
...
The observed cyclic component is indistinguishable from finite-sample noise. No statistically resolved triple forms a directed cycle, and bootstrap rankings are stable. To compare two models A and B directly, we use the simpler finish-above rate: among games containing both models on the same deal, how often does A finish above B? This paired statistic removes much more deck variance than comparing two marginal Elo intervals.
...
We select step 1,200 because it is fully legal and performs better across the frozen panel, including against Claude Opus 4.6. The three-copy self-play mean is only a sanity check: when every seat changes together, total score cannot show whether one checkpoint is stronger than another against a fixed opponent.

## self-play results

Across a reconstructed audit set of seven mainline GRPO configurations, the two runs with direct frozen-SFT tracking remain flat. Several others improve against a fixed scripted heuristic, demonstrating opponent-specific learning. Because five configurations did not record the same stationary anchor, those gains do not measure transfer.
...
None of these results rules out a richer league design. In the directly anchored heterogeneous run and the later weak-KL ablation, opponent mixing and weaker KL likewise fail to produce a positive anchor trend.
...
The heterogeneous run completes 100 optimizer steps with 192 games per rollout generation. Together, these checks make a dead adapter, broken reward ordering, or format collapse poor explanations for the frozen-anchor null.
...
Under relative-margin self-play, all three copies benefit from the same favorable deal, which suppresses most common board advantage. The fixed-heuristic probe uses strategically different policies, allowing repeated outcomes to recover more of the position’s value.

The probe argues against learning $V(I)$ from these particular relative-margin labels; it does not rule out state values in general. An action-conditioned value $Q(I,a)$ could still distinguish bids within the same state.
...
Across four split seeds, four-fold whole-game cross-validation fits small structured models to multi-action states from the $M=64$ reference labels. The table reports ranges across split seeds; “uplift” is offline $Q$ improvement over the SFT candidate mixture, not live game margin.
...
Even under the preregistered gates, every neural run misses the 55% argmax threshold and the 40% linear-bid baseline. The critic learns coarse ordering without a reliable action-value surface.
...
We check ordinary chosen-action calibration on the 150 held-out live games used for the myopic-selector evaluation. Across 1,761 seat-0 treasure bids against two Gemini 3 Flash opponents, mean predicted win probability is 40.3% and empirical win frequency is 36.2%. The fixed-decile expected calibration error is 5.4 percentage points and the Brier score is 0.094. The model is close at the extremes but overpredicts wins in several middle-probability bins.
...
<strong>Key finding.</strong> Improving one-auction expected value harmed the full game because myopic spending depleted liquidity. Extra loan repayment and foregone investment return together cost 10.9 points per game, overwhelming the local asset gain. The intervention also raised early clearing prices by 1.48 coins and lowered late prices by 1.04 after budgets drained.
</div>
...
The archived live choice, $\lambda=0.5,\delta=2$, lies in the original credible-footprint region. In the recovered sweep, it yields +22.62 (standard error 2.23) with 4.33 deviations per game. Live evaluation, not the sweep, confirms the choice.
...
Removing the selector entirely, the distilled weights improve raw paired margin by +7.97 points over SFT, with a 95% confidence interval of [2.70, 13.25]. This comparison uses 150 paired seeds against two Gemini 3 Flash opponents. Control wins 29.3% with mean margin −12.83; the distilled weights win 40.7% with mean margin −4.85.

<details class="collapsible-section">

<summary><strong>preregistered gate and model-assisted estimate</strong></summary>

<span id="preregistered-gate"></span>

The raw effect has $t=2.96$. The control-variate-adjusted improvement is +7.42 [2.56, 12.29], $t=2.99$. The preregistered gate requires a point estimate of at least +4 and a positive effect at $p<.05$; both conditions pass. Interval exclusion of +4 lies outside that criterion, and the lower bound cannot support that stronger claim. The 73% point-estimate ratio below compares separate selector and distillation experiments.
...
The optional selector produces +9.44 [4.55, 14.33] in a seat-rotated 150-pair test, with descriptive per-seat effects of +7.26, +10.94, and +10.12 over 50 pairs each. Against two Claude Sonnet 4.5 opponents, a 50-pair screen is +25.12 [13.78, 36.46]. Against Gemini 3.1 Pro, it is +6.10 [−4.67, 16.87], directionally positive but underpowered. These tests evaluate the selector on top of the distilled policy; the weights-only artifact is unchanged.
...
Across seven audited GRPO configurations, the two runs with direct frozen-SFT tracking stay flat. Several other configurations improve against a fixed scripted heuristic, but they do not record the same transfer measure. This separation is why stationary comparators matter: frozen-SFT anchors, fixed-opponent gates, and non-co-adapting panels answer different questions and should not be conflated.
...
In the runs where transfer is directly tracked, relative outcome rewards did not produce measured transfer across the tested opponent populations. Policy-generated bid logs—from external-model, mixed-policy, and self-play rollouts—provide the useful learning signal: they expose price distributions, make liquidity cost explicit, and yield targets that can be distilled.
