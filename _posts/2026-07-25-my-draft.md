...
Megagem is a multiplayer, imperfect-information auction game developed by Jane Street. In our three-player, general-sum experiments, self-play opponents and relative outcome rewards produce no measured transferable improvement: some policies learn opponent-specific exploits, while the runs with direct frozen-SFT anchors stay flat. We motivate self-play, therefore, primarily through a data generation lens, which support a conditional market model. We begin with Qwen3-4B-Instruct, train it with SFT, then distill a paced and calibrated distributional bidder through expert iteration. The result is a specialist 4B, beating almost all frontier models (GPT-5.5, Claude Opus 4.8, Gemini 3 Flash, etc) in paired games.
...
|---|---:|
| snapshot replication | +4.64 [2.68, 6.57] | 
| no-API replication | +4.36 [2.89, 5.88] |
| long DAPO | +16.48 [14.26, 18.93] |
| seat rotation | +4.04 [−3.88, 11.88] |
| API mix | −2.07 [−3.89, −0.31] |
| Flash mix | −1.16 [−3.22, 0.89] |
| heterogeneous pool | +0.65 [−1.14, 2.43] |
