...
* **Splendor:** an engine-building game in which players collect gem tokens, spend them on development cards that make later purchases cheaper, and race to the most points. The shared cards and objectives make it difficult to think between improving your own position, preserving flexibility, and denying an opponent the resource or bonus they need.
...
Ordinary policy-gradient self-play can still cycle (think like rock-paper-scissors) in the latest policy. Practical systems therefore often average policies or train with fictitious self play, exploiters, and wider populations, and these mechanisms can stop the fresh policy from forgetting old weaknesses.
...
Step 1,200 becomes the obvious checkpoint choice. The self-play mean should only be considered as sanity check because of the co-evolving argument, and it still shows that the model hasn't degenerated.
...
Because first-place rate among three evolving, co-adapting copies remains approximately $\frac1{3}$ regardless of how much the policy evolves, population-based reward is not a sufficient. Instead, we need an anchor, and the initial SFT policy is the perfect choice.
...
A histogram gradient-boosted regressor predicts each opponent seat’s conditional mean bid $\hat\mu_s(x)$ from 16 public features (e.g. coin count, lot size and displayed value, recent clearing prices, and tiebreak position). The frozen model uses 400 iterations, learning rate 0.05, depth 4, and $L_2$ regularization 1.0. We group 5-fold cross-validation by paired seed, and we fit on ~3.6k Gemini 3 Flash bid rows (compared to the earlier ~3.2k). We observe a grouped out-of-fold MAE is 1.230 coins with bias +0.013, with 69.4% of predictions land within one coin.
...
Zero regret can be unattainable when the budget cannot beat a profitable clearing price, or when $V>p$ but losing the tiebreak makes the cheapest winning bid $p+1$ instead of $p$. In fact, even the best oracle can therefore have positive regret because it is still limited by the coin liquidity is has.

To isolate the price model, every counterfactual bidder receives realized terminal gem value $V$ and no mission bonus. Later, our selectorreplaces that privileged value with $\hat V$ plus exact mission value.
...
Our selector first samples an ordinary SFT response, then calculates the EV curve for every legal bid and argmaxs to get $b^*$. To prevent weak analytic preferences from overwriting coherent LLM behavior, it deviates only when the estimated improvement in immediate expected value is at least one coin:
...
Improving one-auction expected value harmed the full game because myopic spending depleted liquidity. Extra loan repayment and foregone investment return together cost 10.9 points per game which overwhelmed the gain won from gems. The intervention also raised early clearing prices by 1.48 coins and lowered late prices by 1.04 after liquidity got deplicate.
...
We therefore sweep for configurations where we make reasonable interventions. The sweep contains a $5\times3$ grid over $\lambda_{\text{coin}}\in\{0.0,0.5,1.0,1.6,2.5\}$ and $\delta\in\{0.0,2.0,5.0\}$ at gate 1, plus three gate-only arms. Its margin-only best is $\lambda=1,\delta=0$ at +26.42 points. But every $\lambda\ge1$ arm enters a proxy-rescue regime where the the selector overbids, and it gains by vetoing proposals that the live LLM does not make.
...
Removing the selector entirely, the distilled weights improve raw paired margin by +7.97 [2.70, 13.25] over SFT. Against Gemini 3 Flash, the control wins 29.3% with mean margin −12.83 with the distilled weights win 40.7% with mean margin −4.85.
...
* **Seat heterogeneity was not tested.** A later Flash test of the selector’s incremental effect spans 300 paired games across 100 seeds. Its overall effect is +4.32 [0.98, 7.66], but seat 0 is flat while seats 1 and 2 are positive whereas the earlier test was positive in every seat.
