...
date: 2026-07-25
image: /public/megagem/megagembench.png
...
The released weights are available on Hugging Face: the [step-1,200 SFT checkpoint](https://huggingface.co/djdumpling/qwen3-4b-instruct-megagem-sft-step1200-v2) and the [style-matched distilled specialist](https://huggingface.co/djdumpling/qwen3-4b-instruct-megagem-distill-para).

This blog would not be possible without the ongoing support of many people. I'm especially grateful to [Johannes Hagemann](https://x.com/johannes_hage?lang=en), [Sebastian Müller](https://x.com/omouamoua), [Christian Reetz](https://x.com/creet_z), and [Peyton Walters](https://x.com/peywalt) for their generous provisioning of credits for compute and access to model APIs. Additionally, thank you to early readers for providing great feedback, including Sebastian Müller, [Varun Srivastava](https://x.com/varunneal/), [Nan Jiang](https://x.com/nanjiangwill), Stephen Xia, Aaron Yu, and Lauren Shen.
...
- [specialist traces](#what-the-specialist-actually-does)
...
<summary><strong>Decision 1: a private hand changes expected value</strong></summary>

<span id="decision-1"></span>

![Decision 1, round 1: a private hand changes the expected value of the red-and-yellow lot.](../public/public/megagem/decision_1.png)
...
<summary><strong>Decision 2: missions create discontinuous value</strong></summary>

<span id="decision-2"></span>

![Decision 2, round 9: a blue-and-green lot completes two missions at once.](../public/public/megagem/decision_2.png)
...
<summary><strong>Decision 3: tiebreak positioning</strong></summary>

<span id="decision-3"></span>

![Decision 3, round 10: bidding from last in tiebreak order costs one extra coin.](../public/public/megagem/decision_3.png)
...
<summary><strong>Decision 4: reveals moving markets</strong></summary>

<span id="decision-4"></span>

![Decision 4, round 11: choosing which private gem to reveal changes public beliefs, not the final supply.](../public/public/megagem/decision_4.png)
...
<summary><strong>Decision 5: loans and liquidity vs face value</strong></summary>

<span id="decision-5"></span>

![Decision 5, round 12: the value of a 20-coin loan comes from the auctions it makes affordable.](../public/public/megagem/decision_5.png)
...
<summary><strong>Decision 6: late-game bidding</strong></summary>

<span id="decision-6"></span>

![Decision 6, round 14: stack sizes determine the covering bid for the final Purple gem.](../public/public/megagem/decision_6.png)
...
The observed cyclic component is indistinguishable from finite-sample noise. No statistically resolved triple forms a directed cycle, and the fitted rankings are stable. To compare two models A and B directly, we simply determine how often A beats B in games where both appear, which removes more deck variance because it is a paired statistic.
...
We train for 1,200 optimizer steps, or roughly three epochs, and retain merged checkpoints from steps 1,000 and 1,200. Training uses a LoRA with rank 32, alpha 64, and dropout 0.05, and learning rate is $2\times10^{-4}$ with cosine decay and 50 warmup steps. We save an adapter every 200 steps.
...
Based on the gameplay panel, we select step 1,200. The self-play mean should only be considered as a sanity check because of the co-evolving argument, and it still shows that the model hasn't degenerated.
...
<summary><strong>GRPO configurations and heuristic diagnostics</strong></summary>
...
<summary><strong>return and advantage construction</strong></summary>
...
<summary><strong>why point predictions are insufficient</strong></summary>
...
We split games 70/10/20 for fitting/calibration/held-out testing/calibration. We use a ReLU MLP ($\mathbb{R}^{11} \to \mathbb{R}^{64} \to \mathbb{R}^{32} \to \mathbb{R}^{7}$) with an $L_2$ coefficient of $10^{-3}$. It trains for at most 300 iterations (patience of 10 and tolerance $1e-4$).
...
<summary><strong>control-variate adjustment</strong></summary>
...
<summary><strong>expert iteration</strong></summary>

Expert iteration alternates between a policy that visits states and an expert that improves the action taken in those states. If the current policy $\pi$ induces a state distribution $I\sim d_\pi$, the expert supplies

$$
a^*(I)=\arg\max_{a\in\mathcal A(I)}U(a\mid I)
$$

and the resulting dataset $\mathcal D_E=\{(I,a^*(I))\}$ trains a new policy by imitation:
...
Here, we run one such iteration. The current SFT policy generates the games, and the paced analytic selector is the expert: its utility $U$ combines predicted lot value, the opponent bid distribution, and the shadow price of liquidity. We then distill its selected bids into the model weights. The selector is absent from the final benchmark, so any retained gain must be produced by the student policy itself.
...
Let's consider the raw final-evaluation trajectories produced by the [distilled model](https://huggingface.co/djdumpling/qwen3-4b-instruct-megagem-distill-para). Below are 3 examples that illustrate smart decisions made by the model and reflect the types of reasoning distilled by expert iteration.

<details class="collapsible-section">

<summary><strong>Trace 1: pricing a mission and the tiebreak together</strong></summary>

Here, the moedl plays seat 0 against GPT-5.5 and Gemini 3.1 Pro. By round 8, it has 14 coins and a collection of Yellow and Purple. A Blue gem would complete the 5-point Blue–Yellow mission, and revealing the Blue in its private hand would move Blue from 4 to 8 points per gem. One opponent has only 6 coins, while the model has tiebreak priority over the other.
...
Recognizing both the value/surplus of Blue while accounting for covering price and coin preservation is what allows it to eventually win the game 93–89–59.
</details>

<details class="collapsible-section">

<summary><strong>Trace 2: recomputing the minimum bid after winning</strong></summary>
...
<summary><strong>Trace 3: borrowing to finish a mission line</strong></summary>
...
This sequence is not enough either: an exact covering bid creates a liquidity shortage which a subsequent loan repairs to fund a second exact covering bid that wins the mission.
