...
We implement RLHF with PPO from scratch and lots of fine-tuning GPT-2 models for movie sentiment classification. We also cover transformer environments, value head initialization, adapative KL control, logit/temperature scaling, and whitening. View the full implementation [here](https://github.com/djdumpling/rl/tree/main/rlhf_transformer).
...
## Future Ideas
 ...
 - **Multi-objective reward**: combine DistilBERT sentiment with coherence, semantic diversity (n-gram + embedding-similarity penalties via SimCSE/SBERT), and discourse consistency.
 - **Optimization alternatives**: augment Adaptive KL with scheduled targets and per-token KL, and benchmark against DPO/IPO (RL-free preference training) built from reward-ranked pairs. Also test trust-region constrained PPO (explicit KL trust region) versus clip-only PPO.
 - **Interpretability**: analyze attention heads and MLP features that activate for negative sentiment (e.g., logit lens and differences) to localize subcircuits. Compare base vs RLHF heads to quantify how steering tokens and prefix tokens shift attribution and causal contribution. (might redo some of the intro interp soon)