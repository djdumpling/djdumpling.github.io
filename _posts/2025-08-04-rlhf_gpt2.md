# Whirlwind of PPO and RLHF for LLMs from scratch (In progress)

We implement RLHF with PPO from scratch, fine-tuning GPT-2 models for movie sentiment classification. We also cover transformer environments, value head initialization, adapative KL control, logit/temperature scaling, and advantage whitening. View the full implementation [here](https://github.com/djdumpling/rl/tree/main/rlhf_transformer).
...
Here's the sketch of the blog so far:
...
- Results
    - Scaling
    - Finetuning against positive bias
    - Other nitty-gritty details

**Coming soon**: more robust and sparse reward penalizing repetitive phrases, other schedulers and optimizers like Muon, advantage whitening, adaptiveKLController, value head initalization, logit scaling, and l2 norm gradient clipping
...
*Figure 12: Log of 5 experiments with `kl_ent` other previous runs as reference. Same running average parameters apply.*

AdaptiveKLControl, logit/temperature, and dense rewards coming soon... :)